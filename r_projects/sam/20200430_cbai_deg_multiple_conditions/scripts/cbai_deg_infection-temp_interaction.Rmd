...
temperature_color = c(D9="#BDBDBD", D12="#737373", D26="#252525")
inf_color = c(infected="#C51B7D", uninfected="#4D9221")


ann_colors <-  list(temperature=temperature_color, infection_status=inf_color)

df1 <- as.data.frame(colData(DEGvst)[c("temperature", "infection_status")]) #make dataframe for column naming 
...
```{r}
DEGPCAdata <- plotPCA(DEGvst, intgroup = c("infection_status", "temperature"), returnData=TRUE)
percentVar <- round(100*attr(DEGPCAdata, "percentVar")) #plot PCA of samples with all data
ggplot(DEGPCAdata, aes(PC1, PC2, color=infection_status, shape=temperature)) + 
  geom_point(size=3) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
