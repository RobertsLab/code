...
Capture vector of warm `sample_id` to use for filtering gene count dataframe:

```{r}
warm_ids <- treatmentinfo %>% 
  filter(treatmentinfo$temperature == "warm") %>% 
  select(sample_id)

# Convert to character vector
warm_ids <- as.vector(as.character(warm_ids$sample_id))

```

Filter treatment info to exclude "warm" temperature - since no uninfected/warm samples. Otherwise, DESeq2 throws the following error:
...
# Remove sample IDs that matched the warm set
gcount <- gcount %>% 
  select(-all_of(warm_ids))
...
Create a DESeqDataSet design from gene count matrix and labels. Here we set the design to look at the interaction of `infection_status` and `temperature` to test for any differences in gene expression across `infection_status` attributed to `temperature`.
...
```{r}
gPCAdata <- plotPCA(gvst, intgroup = c("infection_status", "temperature"), returnData=TRUE)
percentVar <- round(100*attr(gPCAdata, "percentVar")) #plot PCA of samples with all data
ggplot(gPCAdata, aes(PC1, PC2, color=infection_status, shape=temperature)) + 
...
treatmentinfo$sample_id <- as.character(treatmentinfo$sample_id)
treatmentinfo
```



Filter treatment info to retain only `infected` - since no uninfected/warm samples.


```{r}
treatmentinfo <- treatmentinfo %>% 
  filter(treatmentinfo$infection_status == "infected")
treatmentinfo
```


Capture vector of infected `sample_id` to use for filtering gene count dataframe:

```{r}
infected_ids <- treatmentinfo %>%
  select(sample_id)

# Convert to character vector
infected_ids <- as.vector(as.character(infected_ids$sample_id))
...
# Remove sample IDs that matched the infected set
gcount <- gcount %>% 
  select(infected_ids)
...
```{r}
temperature_color = c(ambient="#6A3D9A", cold="#1F78B4", warm="#FF7F00")
gPCAdata <- plotPCA(gvst, intgroup = c("temperature", "day", "infection_status"), returnData=TRUE)
percentVar <- round(100*attr(gPCAdata, "percentVar")) #plot PCA of samples with all data
ggplot(gPCAdata, aes(PC1, PC2, color=temperature, shape=day)) + 
  geom_point(size=4, alpha = 6/10) +
...
temperature_color = c(ambient="#BDBDBD", cold="#737373", warm="#252525")
inf_color = c(infected="#C51B7D")


ann_colors <-  list(temperature=temperature_color)

df1 <- as.data.frame(colData(DEGvst)[c("temperature")]) #make dataframe for column naming 
...
```{r}
DEGPCAdata <- plotPCA(DEGvst, intgroup = c("temperature", "day"), returnData=TRUE)
percentVar <- round(100*attr(DEGPCAdata, "percentVar")) #plot PCA of samples with all data
ggplot(DEGPCAdata, aes(PC1, PC2, color=temperature, shape=day)) + 
  geom_point(size=3) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
  coord_fixed()
