---
title: "Identification of differentially expressed transcripts in G.macrocephalus"
author: "Sam White"
date: "2024-05-24"
output: 
...
# Use [Ballgown](https://github.com/alyssafrazee/ballgown) for identification of differentially expressed isoforms in *G.macrocephalus* gonad tissue exposed to different temperatures.

REQUIRES Linux-based system to run all chunks properly; some chunks will
not work on Mac OS!
...
-   `diffexpressed_col`: Column indicating if gene/transcript is
    differentially expressed (i.e. "No" or != "No")

-   `gene_label_col`: Column with gene label for differentially
    expressed gene/transcript.

-   `title`: Title for plot.

-   `color_labels`: Labels for legend for data point colors. Three
    labels are required.

    -   Order is important! As follows:

    1.  Blue: Differentially expressed (p/q-values \<= 0.05). Higher in
        "left" side of comparison.
    2.  Red: Differentially expressed (p/q-values \<= 0.05). Higher in
...
--accept "*.ctab,*checksums.md5" https://gannet.fish.washington.edu/Atumefaciens/20240522-gmac-HISAT2-stringtie-gffcompare-RNAseq/
```

# Verify checksums

NOTE: Warnings are expected, as the checksums files have checksums for
files that are not downloaded for this project.
...
# Generate a genes BED file, if needed

### Download NCBI GFF

```{r download-ncbi-gff, engine='bash', eval=TRUE}
if [ ! -f "./data/GCF_031168955.1_ASM3116895v1_genomic.gff" ]; then
  wget \
  --directory-prefix ./data \
  --quiet \
https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/031/168/955/GCF_031168955.1_ASM3116895v1/GCF_031168955.1_ASM3116895v1_genomic.gff.gz

  gunzip ./data/GCF_031168955.1_ASM3116895v1_genomic.gff.gz
fi

head -n 15 ./data/GCF_031168955.1_ASM3116895v1_genomic.gff
```

### Create genes BED
```{r create-genes-bed, engine='bash', eval=TRUE}
if [ ! -f "./data/20240524-gmac-GCF_031168955.1_ASM3116895v1_genomic.genes.bed" ]; then
  /home/sam/programs/mambaforge/envs/gffutils-0.12.0_env/bin/gtf_extract \
  --feature gene \
  --fields=chrom,start,end,ID,strand \
  --gff ./data/GCF_031168955.1_ASM3116895v1_genomic.gff \
  | awk 'BEGIN{FS=OFS="\t"}{$5 = 0 OFS $5}1' \
  > ./data/20240524-gmac-GCF_031168955.1_ASM3116895v1_genomic.genes.bed
  
  # Count gene features via GFFutils
  echo "GFFutils number of extracted genes:"
  /home/sam/programs/mambaforge/envs/gffutils-0.12.0_env/bin/gtf_extract --feature gene --fields=ID --gff ./data/GCF_031168955.1_ASM3116895v1_genomic.gff | wc -l

  echo ""

  # Count gene features via awk
  echo "awk number of extracted genes:"
  awk '$3 == "gene" { print $0 }' ./data/GCF_031168955.1_ASM3116895v1_genomic.gff | wc -l
  echo ""
  echo "--------------------------------------------"
  echo ""
fi

head -n 15 ./data/20240524-gmac-GCF_031168955.1_ASM3116895v1_genomic.genes.bed

echo "--------------------------------------------"
echo ""
echo "Number of genes:"
wc -l ./data/20240524-gmac-GCF_031168955.1_ASM3116895v1_genomic.genes.bed
```



# Read in *G.macrocephalus* genes BED file

```{r read-in-genes-BED}
genes_BED <- read.table(file = "./data/20240524-gmac-GCF_031168955.1_ASM3116895v1_genomic.genes.bed")
...
bg <- ballgown(dataDir="./data/ballgown/", samplePattern='^\\d{1,3}$', meas='all')
bg
```

# Download and filter metadata file

Filtered metadata will be used to create a phenotype dataframe needed
for Ballgown differential expression analysis.


```{r create-dataframes-for-ballgwon-pData}
# Read in metadata file from URL
sample_metadata_full <- read.csv("https://raw.githubusercontent.com/RobertsLab/project-cod-temperature/main/data/DESeq2_Sample_Information.csv")

# View full metadata
head(sample_metadata_full)

# Subset metadata in preparation of creating pData data frame far Ballgown
# Sort by `sample_number` to ensure matches directory structure (required for Ballgown)
sample_metadata_subset <- sample_metadata_full %>% 
  select(sample_number, temp_treatment, tissue_type) %>%
  filter((temp_treatment == 9 | temp_treatment == 16) & tissue_type == "Liver") %>% 
  arrange(sample_number)

# Set sample names as characters instead of integers
sample_metadata_subset$sample_number <- as.character(sample_metadata_subset$sample_number)

# View subsetted metadata
str(sample_metadata_subset)
...
# Rotate data frame
# Creates a "temp" column by pulling last character from each library name (M/F)
fpkm_df_pivot <- pivot_longer(
  fpkm_df, cols = starts_with("FPKM"), 
  names_to = "library") %>% 
  mutate(temp = (str_sub(library, -1)
                )
         )
head(fpkm_df_pivot)


# Sort data frame by temp
fpkm_df_pivot_sorted <- fpkm_df_pivot %>% arrange(temp)
head(fpkm_df_pivot_sorted)

# Set unique library names as vector
# Will be used to group boxplot by temp
fpkm_libraries_sorted_unique <- (unique(fpkm_df_pivot_sorted$library))
head(fpkm_libraries_sorted_unique)

# Re-order data frame by temp-sorted data
...
# Grouped by temp
ggplot(fpkm_df_pivot, aes(library, value, color = )) +
  geom_boxplot() +
...
# Identify differentially expressed transcripts (DETs) between temperatures
...
# Identify DETs
DET_temp_stat_results = stattest(bg, feature='transcript', meas='FPKM', covariate="temp_treatment", getFC = TRUE)
head(DET_temp_stat_results)

# Add gene names
DET_temp_stat_results <- data.frame(geneNames=geneNames(bg),
geneIDs=geneIDs(bg), DET_temp_stat_results)

# Filter based on p-value and q-value
DET_temp_filtered_stat_results <- filter(DET_temp_stat_results, pval <= pvalue & qval <= qvalue)
head(DET_temp_filtered_stat_results)

# Filter based on p-value ONLY
DET_temp_pvalOnly_filtered_stat_results <- filter(DET_temp_stat_results, pval <= pvalue)
head(DET_temp_pvalOnly_filtered_stat_results)



# Merge with full table to get subset of just differentially expressed isoforms
merged_DET_temp_filtered <- merge(x = DET_temp_stat_results, y = whole_tx_table, by.x = "id", by.y = "t_id")
head(merged_DET_temp_filtered)
...
merged_DET_temp_filtered <- merged_DET_temp_filtered %>% mutate(start = start - 1)
head(merged_DET_temp_filtered)

# Filter for female up-regulated transcripts (i.e. fold-change < 1)
# Convert to BED coordinate system, which is 0-based start
# and end EXCLUSIVE; so need to subtract 1 from start coordinates ONLY
DET_temp_filtered_female <- filter(merged_DET_temp_filtered, fc < 1) %>% mutate(start = start - 1)
head(DET_temp_filtered_female)

# Filter for male up-regulated transcripts (i.e. fold-change > 1)
# Convert to BED coordinate system, which is 0-based start
# and end EXCLUSIVE; so need to subtract 1 from start coordinates ONLY
DET_temp_filtered_male <- filter(merged_DET_temp_filtered, fc > 1) %>% mutate(start = start - 1)
head(DET_temp_filtered_male)

# Count number of DET.
count_DET_temp_stat_results <-  nrow(DET_temp_stat_results)

# Print number of DET
cat("Number of", comparison, "DET with p-values and q-values <= ", pvalue, ":", count_DET_temp_stat_results, "\n")

# Count number of female DET.
count_DET_temp_filtered_female <-  nrow(DET_temp_filtered_female)

# Print number of female DET
cat("Number of female DET with p-values and q-values <= ", pvalue, ":", count_DET_temp_filtered_female, "\n")

# Count number of male DET.
count_DET_temp_filtered_male <-  nrow(DET_temp_filtered_male)

# Print number of male DET
cat("Number of male DET with p-values and q-values <= ", pvalue, ":", count_DET_temp_filtered_male, "\n")
```

## Write 9C vs 16C DET dataframes to files

```{r write-DETs-9C-vs-16C-to-files}
write.csv(
  DET_temp_pvalOnly_filtered_stat_results, 
  file = "DET_temp_pvalOnly_filtered_stat_results_p-0.05.csv", 
  quote = FALSE, 
  row.names = FALSE)
...
write.table(
  ...
  file = "../output/18-differential-gene-expression-ballgown/DET_temp_filtered_p-0.05_q-0.05.bed",
  sep = "\t",
  col.names = FALSE,
  row.names = FALSE,
  quote = FALSE
...
write.table(
  ...
  file = "../output/18-differential-gene-expression-ballgown/DET_temp_female_filtered_p-0.05_q-0.05.bed",
  sep = "\t",
  col.names = FALSE,
  row.names = FALSE,
  quote = FALSE
...
write.table(
  ...
  file = "../output/18-differential-gene-expression-ballgown/DET_temp_male_filtered_p-0.05_q-0.05.bed",
  sep = "\t",
  col.names = FALSE,
  row.names = FALSE,
  quote = FALSE
...
# Identify differentially expressed genes (DEGs) between temperatures
...
DEG_temp_stat_results = stattest(bg, feature='gene', meas='FPKM', covariate="temp_treatment", getFC = TRUE)
head(DEG_temp_stat_results)

# Filter based on p-value and q-value
DEG_temp_filtered_stat_results <- filter(DEG_temp_stat_results, pval <= pvalue & qval <= qvalue)
head(DEG_temp_filtered_stat_results)

# Filter based on p-value ONLY
DEG_temp_pvalOnly_filtered_stat_results <- filter(DEG_temp_stat_results, pval <= pvalue)
head(DEG_temp_pvalOnly_filtered_stat_results)

# Merge with full table to get subset of just differentially expressed genes
# and their corresponding transcripts.
merged_DEG_temp_filtered <- merge(x = DEG_temp_filtered_stat_results, y = whole_tx_table, by.x = "id", by.y = "gene_id")
head(merged_DEG_temp_filtered)

# Merge with BED file to get DEGs WITHOUT associated transcripts
merged_DEG_temp_filtered_genes_only <- merge(x = DEG_temp_filtered_stat_results, y = genes_BED, by.x = "id", by.y = "name")
head(merged_DEG_temp_filtered_genes_only)


# Convert to BED coordinate system, which is 0-based start
# and end EXCLUSIVE; so need to subtract 1 from start coordinates ONLY
merged_DEG_temp_filtered_ <- merged_DEG_temp_filtered %>% mutate(start = start -1)
head(merged_DEG_temp_filtered)

# Filter for female up-regulated genes (i.e. fold-change < 1)
DEG_temp_filtered_female <- filter(merged_DEG_temp_filtered_genes_only, fc < 1)
head(DEG_temp_filtered_female)

# Filter for male up-regulated genes (i.e. fold-change < 1)
DEG_temp_filtered_male <- filter(merged_DEG_temp_filtered_genes_only, fc > 1)
head(DEG_temp_filtered_male)

# Count number of DEG.
count_merged_DEG_temp_filtered_genes_only <-  nrow(merged_DEG_temp_filtered_genes_only)

# Print number of DEG
cat("Number of ", comparison, " DEG with p-values and q-values <= ", pvalue, ":", count_merged_DEG_temp_filtered_genes_only, "\n")

# Count number of female DEG.
count_DEG_temp_filtered_female <-  nrow(DEG_temp_filtered_female)

# Print number of female DEG.
cat("Number of female DEG with p-values and q-values <= ", pvalue, ":", count_DEG_temp_filtered_female, "\n")

# Count number of male DEG.
count_DEG_temp_filtered_male <-  nrow(DEG_temp_filtered_male)

# Print number of male DEG.
cat("Number of male DEG with p-values and q-values <= ", pvalue, ":", count_DEG_temp_filtered_male, "\n")
```
...
write.csv(
  DEG_temp_pvalOnly_filtered_stat_results, 
  file = "DEG_temp_pvalOnly_filtered_stat_results_p-0.05.csv", 
  quote = FALSE, 
  row.names = FALSE)
...
write.table(
  (merged_DEG_temp_filtered_genes_only %>% 
    select(chr, start, end, id, score, strand)
    ), 
  file = "../output/18-differential-gene-expression-ballgown/DEG_temp_filtered_p-0.05_q-0.05.bed",
  sep = "\t",
  col.names = FALSE,
  row.names = FALSE,
  quote = FALSE
...
write.table(
  (
    DEG_temp_filtered_female %>% 
    select(chr, start, end, id, score, strand)
    ),
  file = "../output/18-differential-gene-expression-ballgown/DEG_temp_female_filtered_p-0.05_q-0.05.bed",
  sep = "\t",
  col.names = FALSE,
  row.names = FALSE,
  quote = FALSE
...
write.table(
  (
    DEG_temp_filtered_male %>% 
    select(chr, start, end, id, score, strand)
    ),
  file = "../output/18-differential-gene-expression-ballgown/DEG_temp_male_filtered_p-0.05_q-0.05.bed",
  sep = "\t",
  col.names = FALSE,
  row.names = FALSE,
  quote = FALSE
...
NOTE: Will not return fold change; not possible in multi-group
comparisons
...
I.e. Are DET based off of alphanumeric comparisons? Uses the
`mutated_sample_metadata_subset` with a `0` in place of `4` in
`TreatmentN` numbers to see if different number of DETs occur. Compare
to results from analysis in chunk above.
