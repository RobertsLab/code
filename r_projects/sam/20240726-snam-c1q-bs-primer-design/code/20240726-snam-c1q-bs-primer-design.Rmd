...
This process also utilizes [pyfaidx](https://github.com/mdshw5/pyfaidx) [@shirley2015] and [samtools](https://www.htslib.org/) [@li2009].
...
```{r save-bash-vars-to-r-vars, eval=TRUE, echo=FALSE, include=FALSE}
# Use system to get the value of left_buffer from Bash
...
Quick explanation:
[Primer3](https://github.com/primer3-org/primer3) requires a specially formatted input file. The file must be formatted similarly to this:
...
Additionally, due to the low complexity of the bisulfite-converted sequence (very few `G`, due to conversion to `U/T`), I've changed the parameters to look for longer primers _and_ lower melting temps than the default settings.
...
# GENOME BISULFITE CONVERSION

```{bash bs-convert-genome, engine='bash', eval=TRUE}
# Load bash variables into memory
source .bashvars
...
# PRIMER SEARCH WITH [EMOBSS PRIMERSEARCH](https://emboss.sourceforge.net/apps/cvs/emboss/apps/primersearch.html)

## Create EMOBSS PrimerSearch Primers File
